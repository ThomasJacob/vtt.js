/* vtt.js - v0.13.0 (https://github.com/mozilla/vtt.js) built on 19-04-2017 */
...
(function (global) {
  ...
  function CueStyleBox(window, cue, styleOptions, customStyle) {
    var isIE8 = (typeof navigator !== "undefined") &&
      (/MSIE\s8\.0/).test(navigator.userAgent);
    var textFont = styleOptions.font;
    var textSize = styleOptions.fontSize;
    ...
    var windowFlashAnimation = null;
    var panelFlashAnimation = null;
    ...
    if (customStyle) {
      ...
      function getCustomStyleValue(obj) {
        return obj.value;
      }

      textFont = getCustomStyleValue(customStyle.textFont);
      textSize = getCustomStyleValue(customStyle.textSize);
      color = getCustomStyleValue(customStyle.textColor);
      textShadow = getCustomStyleValue(customStyle.textEdgeStyle);

      backgroundColor = getCustomStyleValue(customStyle.backgroundColor);

      windowColor = getCustomStyleValue(customStyle.windowColor);
      if (customStyle.windowFlashAnimation) {
        windowFlashAnimation = getCustomStyleValue(customStyle.windowFlashAnimation);
      }

      if (customStyle.panelFlashAnimation) {
        panelFlashAnimation = getCustomStyleValue(customStyle.panelFlashAnimation);
      }
    }
    ...
    styles = {
      ...
      position: "absolute",
      display: "inline-block",
      animation: windowFlashAnimation
    };
  ...
  BoxPosition.getDivSimpleBoxPosition = function (obj) {
  ...
  WebVTT.processCues = function (window, cues, overlay, customSettings) {
    if (!window || !cues || !overlay || !customSettings) {
      return null;
    }
    ...
    var boxPositions = [],
      containerBox = BoxPosition.getDivSimpleBoxPosition(paddedOverlay),
    ...
    (function () {
      ...
      for (var i = 0; i < cues.length; i++) {
        ...
        styleBox = new CueStyleBox(window, cue, styleOptions, customSettings);
